...
class FileName:
    ...
    def updateFn(self):
        self.fn = '%s/%s_%s_%s_%03d' % (self._directory, self._kidID, \
                self._H[self.indH], self._Z[self.indZ], \
                self._K[self.indH][self.indZ])
        self.fn = self.fn + self._delim + self._proto
        self.fn = self.fn + self._ext
...
class FileIndex():
    ...
    def __init__(self, directory = './'):
        ...
        self._fileRegex = '_[LR]_[0-4]_\d{3}\_.*.bmp$'
        self._Hindex = 1 # L or R
        self._Findex = 2 # 0-4
        self._Sindex = 3 # 00-99
